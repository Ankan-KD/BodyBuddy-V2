...
// ─── Screens ────────────────────────────────────────────────────────────────
// "auth"       → the main login / signup form
// "signup_otp" → 8-digit OTP verification after manual email signup
// "fp_email"   → forgot password: enter email
// "fp_otp"     → forgot password: enter OTP
// "fp_newpass" → forgot password: enter new password
// "fp_done"    → forgot password: success

type Screen = "auth" | "signup_otp" | "fp_email" | "fp_otp" | "fp_newpass" | "fp_done";
...
function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 2.09 2.18 4.93l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  );
}
    ...
    return (
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)] mb-6">
          We sent an 8-digit code to <strong>{email}</strong>. Check your inbox (and spam folder).
        </p>
        <form onSubmit={handleVerifyOTP} className="space-y-3">
          ...
          />
          {error && <p className="text-xs text-ember-400">{error}</p>}
          <Button type="submit" size="lg" className="w-full" disabled={loading || otpCode.length < 8}>
...
// After manual email signup (name + email only — NO password collected yet),
// the user verifies their 8-digit OTP, then the app marks their email as
// verified (pending_email_verify=false) and sends them to /set-password.
//
// FLOW: Email signup → OTP → completeSignupVerify (clears pending flag) → /set-password
//
// Google signup NEVER enters this component — Google has already verified
// the email. Google users go directly: Google auth → /set-password.
...
function SignupOtpFlow({
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [cooldown, setCooldown] = useState(60); // starts cooling from initial send
  ...
  async function handleResend() {
    ...
    setError(null);
    setLoading(true);
    const { error: err } = await sendOTP({ purpose: "signup_verify", email });
    setLoading(false);
    if (err) { setError(err); return; }
    setCooldown(60);
    setOtpCode("");
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    // Step 1: verify the 8-digit OTP server-side → get verifyToken
    const { error: serverErr, verifyToken } = await verifyOTPServer({
      purpose: "signup_verify",
      code: otpCode,
      email,
    });
    if (serverErr || !verifyToken) {
      setLoading(false);
      setError(serverErr ?? "Verification failed. Please try again.");
      return;
    }

    // Step 2: consume the verifyToken to clear pending_email_verify flag
    if (!session?.access_token) {
      setLoading(false);
      setError("Session expired — please sign in again.");
      return;
    }
    const { error: completeErr } = await completeSignupVerify({
      email,
      verifyToken,
    ...
    // Both steps succeeded — OTP verified, flag cleared server-side.
    // Flip our local copy of pendingEmailVerify BEFORE navigating, so the
    // Gate already knows this user isn't pending anymore the instant
    // /set-password mounts, instead of briefly bouncing back to /login
    // on stale state (see the note above AuthProvider's account-state
    // effect in src/lib/auth.tsx for the full story).
    markEmailVerified();
    onVerified();
  }
  ...
  return (
    <div>
      ...
      <h2 className="font-display text-xl font-semibold mb-1">Verify your email</h2>
      <p className="text-sm text-[var(--text-muted)] mb-6">
...
export default function LoginPage() {
  const { signInWithEmail, signUpWithEmail, signInWithGoogle, configured, session } = useAuth();
  ...
  async function submit(e: React.FormEvent) {
    ...
    // ── Manual Email Signup ────────────────────────────────────────────
    // Collect name + email only. A random temp password is used internally.
    // The real password is set AFTER OTP email verification on /set-password.
    const tempPassword = crypto.randomUUID() + "!Aa1";
    setLoading(true);
    const result = await signUpWithEmail(email, tempPassword, name);
    setLoading(false);
    if (result.error) { setError(result.error); return; }

    // Send 8-digit OTP to verify the email address.
    const { error: otpErr } = await sendOTP({ purpose: "signup_verify", email });
    if (otpErr) {
      setError(`Account created but we couldn't send a verification email: ${otpErr}. Please try logging in and verifying your email.`);
      return;
    }
  ...
  async function google() {
    ...
    // On success, browser is redirected to Google — we never reach here.
    if (result.error) { setError(result.error); setGoogleLoading(false); }
  }

  // OTP verified + pending flag cleared → navigate to /set-password
  ...
  if (screen === "fp_email" || screen === "fp_otp" || screen === "fp_newpass" || screen === "fp_done") {
    return (
      <div className="min-h-dvh flex flex-col justify-center px-6 py-10">
        <div className="mx-auto w-full max-w-sm">
          <div className="flex flex-col items-center mb-8">
          ...
          <ForgotPasswordFlow onBack={() => { setScreen("auth"); setMode("signin"); }} />
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-dvh flex flex-col justify-center px-6 py-10">
      <div className="mx-auto w-full max-w-sm">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {/* Password only shown for login — signup sets password after OTP. */}
          {mode === "signin" && (
            <div className="space-y-1">
              ...
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={() => { setScreen("fp_email"); setError(null); }}
          ...
          {mode === "signup" && (
            <p className="text-xs text-[var(--text-muted)] px-1">
              We&apos;ll email you an 8-digit code to verify your address, then you&apos;ll set a password.
            </p>
          )}
